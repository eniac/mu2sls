...
assert ip is not None, "LOAD_BALANCER_IP is not set"

movie_titles = ["Avengers: Endgame",
...
def once_tree() -> float:
    ...
    headers = {"Content-Type": "application/json",
               "Host": "callertxn.default.example.com"}
    start = time.perf_counter()
    r = requests.post(f"http://{ip}/req", json={
        "args": [value],
        "req_id": str(uuid4())
    }, headers=headers)
    res = r.status_code
...
def once_chain() -> float:
    value = random.randint(0, 999)
    headers = {"Content-Type": "application/json",
               "Host": "caller1.default.example.com"}
    start = time.perf_counter()
    r = requests.post(f"http://{ip}/req", json={
        "args": [value],
        "req_id": str(uuid4())
...
def once_hotel() -> float:
    ...
    start = time.perf_counter()
    r = requests.post(f"http://{ip}/req", json={
        "args": [str(user_id), str(flight_id), str(hotel_id)],
        "req_id": str(uuid4())
    }, headers=headers)
    end = time.perf_counter()
    return end - start
...
def once(name: str) -> float:
    if name == "tree":
        return once_tree()
    elif name == "single-stateful":
        return once_single()
    elif name == "chain":
        return once_chain()
    elif name == "media-service-test":
        return once_media()
    elif name == "hotel-reservation":
        return once_hotel()
    else:
...
def main(name: str):
    ...
    lats = []
    for i in range(200):
        lats.append(once(name))
    g_end = time.perf_counter()
    dura = g_end - g_start
    print()
    print(f"throughput: {200 / dura}")
